...
sub simple_convert {
    my ($value, $unit) = @_;
    apply_all(sub { $value ||= shift()->simple_convert($value, $unit); });
    return $value;
}

sub canonical {
    my ($v) = @_;
    apply_all(sub { $v = shift()->canonical($v); });
    return $v;
}
...
sub reduce {
    my ($v) = @_;
    $DB::single = 1;
    apply_all(sub { $v = shift()->canonical($v, 'reduce'); });
    return $v;
}
...
my %short_prefixable_user_type = ( bps => [ 'per', 'bit', 'sec' ],
);
